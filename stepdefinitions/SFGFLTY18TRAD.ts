...
import {
	SFGFLTY18TRADhomePageObject
} from "../pages/SFGFLTY18TRADhomePage";
import {
...
import {
	SFGFLTY18TRADJobCompletionPageObject
} from "../pages/SFGFLTY18TRADJobCompletion";
import {
	config
} from "../config/config";
import {
...
const home: SFGFLTY18TRADhomePageObject = new SFGFLTY18TRADhomePageObject();
...
const SFGFLTY18TRADComplete: SFGFLTY18TRADJobCompletionPageObject = new SFGFLTY18TRADJobCompletionPageObject();
...
Then('I should see SFGFLTY18TRAD DETERMINE FAULT ACTIVITY section', async function () {
	await utility.wait(5000);
	await SFGFLTY18TRADComplete.determintfaultActivity();
});
...
When('I fill the SFGFLTY18TRAD determine faulty activity fields with values', async function () {
	await SFGFLTY18TRADComplete.fill14commsdtls();
	await SFGFLTY18TRADComplete.captureevidsuspectedtamp();
	await SFGFLTY18TRADComplete.fill15commsdtls();
});
...
Then('I should see SFGFLTY18TRAD current PPMID section', async function () {
	await SFGFLTY18TRADComplete.CurrentPPMIDSectiondispaly();
	await utility.wait(1000);
});
...
When('I fill the SFGFLTY18TRAD current PPMID details fields with the values', async function () {
	await SFGFLTY18TRADComplete.fillCurrentPPMIDSection();
});
...
Then('I should finally submit SFGFLTY18TRAD job',async function () {
	await SFGFLTY18TRADComplete.FinalSubmission();
