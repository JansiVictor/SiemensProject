...
export class SFEFLTY19homePageObject {
	...
	constructor() {
	this.selectLink = element.all(by.xpath('//*[starts-with(@id,"SelectJobFAULTY 19")]'));
    this.selectLink2 = element.all(by.xpath('//*[starts-with(@id,"SelectJobFAULTY 19")]'));
	...
	public clickOnTheSelectLink() {
		...
		list.count().then(function (promiseResult) {
			...
			var selectlatest = element(
				by.xpath('((//*[starts-with(@id,"SelectJobFAULTY 19")]))' + '[' + [size] + ']')
			);
			selectlatest.getText().then(function (selectText) {
				console.log("find select link text  " + selectText);
			});
			console.log("link " + ('((//*[starts-with(@id,"SelectJobFAULTY 19")]))' + '[' + [size] + ']'));
