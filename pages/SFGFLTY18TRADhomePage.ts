...
export class SFGFLTY18TRADhomePageObject {
  ...
  public selectLink2: ElementArrayFinder;
	...
	constructor() {
	this.selectLink = element.all(by.xpath('//*[starts-with(@id,"SelectJobFAULTY 18")]'));
    this.selectLink2 = element.all(by.xpath('//*[starts-with(@id,"SelectJobFAULTY 18")]'));
	...
	public clickOnTheSelectLink() {
		...
		list.count().then(function (promiseResult) {
			...
			var selectlatest = element(
				by.xpath('((//*[starts-with(@id,"SelectJobFAULTY 18")]))' + '[' + [size] + ']')
			);
			selectlatest.getText().then(function (selectText) {
				console.log("find select link text  " + selectText);
			});
			console.log("link " + ('((//*[starts-with(@id,"SelectJobFAULTY 18")]))' + '[' + [size] + ']'));
