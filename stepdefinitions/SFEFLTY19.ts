...
import {
	SFEFLTY19homePageObject
} from "../pages/SFEFLTY19homePage";
import {
...
const home: SFEFLTY19homePageObject = new SFEFLTY19homePageObject();
...
const SFEFLTY19Complete: SFGFLTY18TRADJobCompletionPageObject = new SFGFLTY18TRADJobCompletionPageObject();
...
Then('I should see SFEFLTY19 DETERMINE FAULT ACTIVITY section', async function () {
	await utility.wait(5000);
	await SFEFLTY19Complete.determintfaultActivity();
});
...
When('I fill the SFEFLTY19 determine faulty activity fields with values', async function () {
	await SFEFLTY19Complete.filleleccommsdtls();
	await SFEFLTY19Complete.captureevidsuspectedtamp();
	await SFEFLTY19Complete.fillFlty19commsdtls();
	await utility.wait(5000);
...
Then('I should see SFEFLTY19 current PPMID section', async function () {
	await SFEFLTY19Complete.CurrentPPMIDSectiondispaly();
...
When('I fill the SFEFLTY19 current PPMID details fields with the values', async function () {
	await SFEFLTY19Complete.fillCurrentPPMIDSectionYes();
});
...
When('I fill the SFEFLTY19 device binding section fields with values',async function () {
  await SFEFLTY19Complete.fillDeviceBindingSection();
...
Then('I should finally submit SFEFLTY19 job',async function () {
	await SFEFLTY19Complete.FinalSubmission();
